...
本ファイルの JSON スキーマ・annotations・bg_prompt・display_text・品質基準は `/gen-script-paper`（論文モード）とも共通の参照元。論文向けの差分は `.claude/commands/generate-script-paper.md` を参照。
...
| **ひっくり返し型** | 意外な結末を1文で完結させる（他者を貶める皮肉は避ける） | 「型落ちのGPUが、最新モデルの性能を上回ってしまいました。」 |
| **緊迫感型** | 時間・速さで緊張感を出す | 「たった7分で、世界最速の記録が塗り替えられました。」 |
| **ループ型** | 最も驚くべき事実を冒頭に置き、疑問で締める | 「1人のエンジニアが、10年かかる作業を1日で終わらせました。なぜそんなことができたのか。」 |

- **最優先**: 記事に具体的な数字・金額・記録があれば hook の冒頭に置く（例: 「たった2枚のGPUで、世界記録が塗り替えられました。」）。主語は Sony・Google など誰もが知る固有名詞にすると掴みが一段強くなる
- **倫理方針**: 特定の企業・個人を貶める、不安・嫌悪を煽る書き方はしない。記事が本当にネガティブな内容の場合も、誇張せず事実として淡々と伝える
- **禁止表現**: 「詳細はこのあと」「続きはこのあと」、セクション末尾の「しかし、一番驚くのはここからです。」のような引き文句。ナレーションは自然な流れで完結させること
...
**倫理方針（最優先）**: 特定の企業・個人を貶める、不祥事・失敗を面白がる、事実を誇張して不安や嫌悪を煽るようなタイトルは作らない。記事が本当にネガティブな内容（不具合・規制・損失など）であっても、誇張せず事実の通りに伝える。「叩き」ではなく「驚き」「発見」「達成」でフックする。

**必ず盛り込む（できれば複数を組み合わせる）:**
1. **具体的な数字・記録・成果**（世界最速 / 19年 / コスト1/10 / 7分 / 15年）。数字は最も強い。
2. **誰もが知る主語**（Sony / Google / Apple / Meta / EU / 中国 等）を文頭近くに置く。
3. **達成・突破・解決・更新など、事実に基づいた強い動詞で言い切る**（達成した / 塗り替えた / 解決した / 実現した など）。過去形・体言止めで言い切る。記事の内容が本当にネガティブな場合のみ、誇張せず事実通りの動詞を使う（中止した / 延期した 等）。
4. 可能なら**物語・人視点**（「〇〇を成し遂げた話」「〇〇に挑んだ1日」）にする。

**勝ちパターン例:**
- `GPUたった2枚で世界最速記録を更新した`（数字＋大企業＋達成）
- `17歳の高校生が数学の未解決問題を解いた`（意外性＋達成）
- `OpenAIの新モデルが推論コストを1/10にした`（大企業＋数字＋改善）
- `偶然ハッカーになって450万円もらった話`（実話＋金額）
- `Googleのエンジニアが10年かかる作業を1日で終わらせた`（大企業＋驚き＋達成）

**避けるべきタイトル:**
- 「〇〇が発表されました」「〇〇社が新しい〇〇機能を発表」のような**説明的な平叙文**（8倍の差がつく最悪パターン）
- 具体物・数字のない抽象タイトル（「〇〇の常識が変わる」）
- 人事・移籍・資金調達など感情の動かないネタ
- **特定の企業・個人を貶める、煽情的・攻撃的な表現**（不祥事や失敗を面白がる論調、誇張された喪失・裏切り表現など）
...
- 例: `"**GPU**たった**2枚**で世界最速記録を更新"`、`"**Google**のエンジニアが**10年**分の作業を1日で終わらせた"`
