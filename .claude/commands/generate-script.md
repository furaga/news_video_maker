...
2. 以下の構成で台本を生成:
   ...
   | `outro` | まとめ・締め | 4〜5秒（約30〜40文字） |

   **重要**: セクションを細かく分けることで、各セクション切り替え時にカードアニメーションが発生し、画面に動きが生まれる。`02_selected.json` に `related_research` フィールドがあれば、その情報を `main_3` や `main_4` に積極的に活用すること。
